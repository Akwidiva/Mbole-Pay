...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const updated = await prisma.withdrawalRequest.update({ where: { id }, data: { status: "PENDING" } })

    return NextResponse.json({ success: true, data: updated })
  } catch (error: any) {
    console.error("Retry withdrawal error:", error)
    return NextResponse.json({ success: false, error: { code: "SERVER_ERROR", message: "Failed to retry withdrawal", details: process.env.NODE_ENV === "development" ? error.message : undefined } }, { status: 500 })
  }
}
